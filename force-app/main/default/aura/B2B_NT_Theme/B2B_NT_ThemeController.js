({
    ...
    scriptsLoaded: function (component, event) {
      ...
      var colorShadesToCreate = [
        {
          base: getComputedStyle(document.body).getPropertyValue(
      ...
      function attachColorVariables(colorsArr) {
        ...
        colorsArr.forEach(function (colorsObj) {
          var baseColor =
            colorsObj.base.indexOf("rgb") == -1
              ? hexToRgb(colorsObj.base)
              : colorsObj.base;
          colorsObj.shades.forEach(function (shadesObj) {
            extravars.style.setProperty(
              "--b2btheme_" + shadesObj.varName,
              getColorShade(
                baseColor,
                shadesObj.brightnessDiff,
                shadesObj.opacity
              )
            );
          });
