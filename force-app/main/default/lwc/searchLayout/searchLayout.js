...
export default class SearchLayout extends LightningElement {
    ...
    addToCartAllEvent(){
        this.template.querySelectorAll('c-search-card').forEach(item => item.addToCartAllAction());
    }

    handleCartChanges() {
        let quantity = 0;
        this.isAddAllButtonDisabled = false;
        let searchCards = this.template.querySelectorAll('c-search-card');
        searchCards.forEach(function(sc) {
            quantity += parseInt(sc.getQuantityValue());
        })
        if(quantity === 0)
            this.isAddAllButtonDisabled = true;
    }
}